 ...
 */
function SettingsModalComponent(props: SettingsModalProps) {
  const {
    is_open,
    api_base_url,
    set_api_base_url,
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal_header">
          ...
          <button className="settings_btn" onClick={on_close}>
            {t(ui_language || 'zh-CN', 'close')}
          </button>
        </div>
        <div className="modal_body">
          <div className="tabs">
            <button
              className={`tab_button ${active_tab === 'ui' ? 'active' : ''}`}
              onClick={() => set_active_tab('ui')}
            >
              {t(ui_language || 'zh-CN', 'tab_ui')}
            </button>
            <button
              className={`tab_button ${active_tab === 'ai' ? 'active' : ''}`}
              onClick={() => set_active_tab('ai')}
            >
              {t(ui_language || 'zh-CN', 'tab_ai')}
            </button>
            <button
              className={`tab_button ${active_tab === 'templates' ? 'active' : ''}`}
              onClick={() => set_active_tab('templates')}
            >
              {t(ui_language || 'zh-CN', 'tab_templates')}
            </button>
          </div>
            ...
            <div className="tab_panel">
              {recent_files.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ color: '#cfcfcf', fontSize: 13, marginBottom: 6 }}>
                    {t(ui_language || 'zh-CN', 'recent_files_label')}
                  </div>
                    ...
                    {recent_files.slice(0, 10).map((p) => (
                      <li key={p} style={{ wordBreak: 'break-all', margin: '4px 0' }}>
                        <button
                          ...
                          onClick={() => on_open_recent && on_open_recent(p)}
                          title={p}
                        >
                          {t(ui_language || 'zh-CN', 'open_label')}
                        </button>
                  ...
                  {clear_recent_files && (
                    <button
                      className="settings_btn"
                      style={{ marginTop: 8 }}
                      onClick={clear_recent_files}
                    >
                      {t(ui_language || 'zh-CN', 'clear_recent')}
                    </button>
                  )}
                </div>
              )}
              <div className="form_row">
                <label className="form_label">
                  {t(ui_language || 'zh-CN', 'editor_font_size')}
                </label>
                <input
              ...
              <div className="form_row">
                <label className="form_label">
                  {t(ui_language || 'zh-CN', 'preview_font_size')}
                </label>
                <input
                  className="settings_input"
                  placeholder="16"
              ...
              <div className="form_row">
                ...
                <select
                  className="settings_input"
                  value={ui_language}
                  onChange={(e) => set_ui_language && set_ui_language(e.target.value)}
                >
              ...
              <div className="form_row">
                ...
                <select
                  ...
                  value={ui_theme}
                  onChange={(e) =>
                    set_ui_theme && set_ui_theme(e.target.value as 'dark' | 'light' | 'system')
                  }
                >
          ...
          {active_tab === 'ai' && (
            <div className="tab_panel">
              ...
              <div style={{ fontSize: 12, color: '#888', marginBottom: 12, lineHeight: 1.5 }}>
                💡 <strong>OpenAI (兼容 API)</strong> 支持所有使用 OpenAI API 格式的服务商：
                <br />
                • DeepSeek, Kimi, 通义千问, 智谱, Gemini, Azure OpenAI, OpenRouter 等
                <br />• 只需修改 <strong>API Base URL</strong> 和 <strong>API Key</strong> 即可使用
              </div>

              {provider === 'openai' && (
                <details style={{ marginBottom: 12, fontSize: 12 }}>
                  <summary style={{ cursor: 'pointer', color: '#6aa0ff', marginBottom: 8 }}>
                    📋 常用配置示例（点击展开）
                  </summary>
                  <div
                    style={{
                      ...
                      padding: '8px 12px',
                      borderRadius: 4,
                      borderLeft: '2px solid #6aa0ff',
                      lineHeight: 1.6,
                      color: '#ccc',
                    }}
                  >
                    <div style={{ marginBottom: 8 }}>
                      <strong style={{ color: '#6aa0ff' }}>DeepSeek</strong>
                      <br />
                      API Base URL:{' '}
                      <code
                        style={{
                    ...
                    <div style={{ marginBottom: 8 }}>
                      <strong style={{ color: '#6aa0ff' }}>Kimi (Moonshot AI)</strong>
                      <br />
                      API Base URL:{' '}
                      <code
                        style={{
                    ...
                    <div style={{ marginBottom: 8 }}>
                      <strong style={{ color: '#6aa0ff' }}>通义千问 (Qwen)</strong>
                      <br />
                      API Base URL:{' '}
                      <code
                        style={{
                    ...
                    <div style={{ marginBottom: 8 }}>
                      <strong style={{ color: '#6aa0ff' }}>智谱 AI (ChatGLM)</strong>
                      <br />
                      API Base URL:{' '}
                      <code
                        style={{
                    ...
                    <div>
                      <strong style={{ color: '#6aa0ff' }}>OpenRouter</strong>
                      <br />
                      API Base URL:{' '}
                      <code
                        style={{
              ...
              <div className="form_row">
                ...
                <input
                  ...
                  placeholder={
                    is_ollama
                      ? t(ui_language || 'zh-CN', 'base_url_ph_ollama')
                      : t(ui_language || 'zh-CN', 'base_url_ph_openai')
                  }
                  value={api_base_url}
              ...
              <div className="form_row">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <input
                    ...
                    placeholder={
                      is_ollama
                        ? t(ui_language || 'zh-CN', 'api_key_ph_ollama')
                        : t(ui_language || 'zh-CN', 'api_key_ph_required')
                    }
                    value={api_key}
                  ...
                  <button
                    className="settings_btn"
                    onClick={on_test}
                    title={t(ui_language || 'zh-CN', 'test_connection')}
                    style={{ padding: '0 12px', whiteSpace: 'nowrap' }}
                  >
                    {t(ui_language || 'zh-CN', 'test')}
                  </button>
                </div>
              </div>
              <div className="form_row">
                ...
                <input
                  ...
                  placeholder={
                    is_ollama
                      ? t(ui_language || 'zh-CN', 'model_ph_ollama')
                      : t(ui_language || 'zh-CN', 'model_ph_openai')
                  }
                  value={model}
              ...
              <div className="form_row">
                <label className="form_label">
                  {t(ui_language || 'zh-CN', 'model_list_label')}
                </label>
                <div style={{ display: 'flex', gap: 8, width: '100%' }}>
                  <button className="settings_btn" onClick={handle_fetch_models}>
                    {loading_models
                      ? t(ui_language || 'zh-CN', 'loading_text')
                      : t(ui_language || 'zh-CN', 'get_models_btn')}
                  </button>
                  <select
                    className="settings_input"
                    value={model}
                  ...
                  >
                    <option value="">
                      {t(ui_language || 'zh-CN', 'select_model_placeholder')}
                    </option>
                    {model_list.map((m) => (
              ...
              {load_error && (
                <div style={{ color: '#f88', fontSize: 12, marginTop: 4 }}>
                  {t(ui_language || 'zh-CN', 'error_prefix')} {load_error}
                </div>
              )}
              <div className="form_row">
                <label className="form_label">
                  {t(ui_language || 'zh-CN', 'system_prompt_label')}
                </label>
                <input
                  className="settings_input"
              ...
              <div className="form_row">
                <label className="form_label">
                  {t(ui_language || 'zh-CN', 'temperature_label')}
                </label>
                <input
                  className="settings_input"
          ...
          {active_tab === 'templates' && (
            <div className="tab_panel">
              <div style={{ color: '#cfcfcf', fontSize: 13, marginBottom: 8 }}>
                {t(ui_language || 'zh-CN', 'ai_menu_title')}
              </div>
              {[
                ...
                {
                  id: 'translate_zh_selection',
                  label: t(ui_language || 'zh-CN', 'translate_zh_sel'),
                },
                {
                  id: 'translate_en_selection',
                  label: t(ui_language || 'zh-CN', 'translate_en_sel'),
                },
              ...
              <div style={{ color: '#cfcfcf', fontSize: 13, margin: '12px 0 6px' }}>
                {t(ui_language as any, 'custom_templates_hint')}
              </div>
              {ai_custom_templates.map((tpl, idx) => (
                ...
                >
                  <div className="form_row">
                    <label className="form_label">
                      {t(ui_language || 'zh-CN', 'title_label_i18n')}
                    </label>
                    <input
                      className="settings_input"
                      value={tpl.title}
                  ...
                  <div className="form_row">
                    <label className="form_label">
                      {t(ui_language || 'zh-CN', 'scope_label_i18n')}
                    </label>
                    <select
                      className="settings_input"
                      value={tpl.scope}
                    ...
                    >
                      <option value="selection">
                        {t(ui_language || 'zh-CN', 'scope_selection')}
                      </option>
                      <option value="document">
                        {t(ui_language || 'zh-CN', 'scope_document')}
                      </option>
                    </select>
                  </div>
                  <div className="form_row" style={{ alignItems: 'flex-start' }}>
                    <label className="form_label">
                      {t(ui_language || 'zh-CN', 'prompt_label_i18n')}
                    </label>
                    <textarea
                      className="settings_input"
                      style={{ height: 100 }}
                  ...
                  <div className="form_row">
                    <label className="form_label">
                      {t(ui_language || 'zh-CN', 'vars_label_i18n')}
                    </label>
                    <div style={{ display: 'flex', gap: 8, width: '100%' }}>
                      <input
                        className="settings_input"
                        placeholder="lang (e.g. zh-CN/en-US)"
                        value={tpl.vars?.lang || ''}
                        onChange={(e) => {
                          const next = [...ai_custom_templates]
                          next[idx] = {
                            ...tpl,
                            vars: { ...(tpl.vars || {}), lang: e.target.value },
                          }
                          set_ai_custom_templates(next)
                        }}
                      />
                      <input
                        className="settings_input"
                        placeholder="style (e.g. concise/formal)"
                        value={tpl.vars?.style || ''}
                        onChange={(e) => {
                          const next = [...ai_custom_templates]
                          next[idx] = {
                            ...tpl,
                            vars: { ...(tpl.vars || {}), style: e.target.value },
                          }
                          set_ai_custom_templates(next)
                        }}
                      />
                  ...
                  <div className="form_row">
                    <label className="form_label">
                      {t(ui_language || 'zh-CN', 'enabled_label_i18n')}
                    </label>
                    <input
                      type="checkbox"
                      checked={tpl.enabled}
                    ...
                    >
                      {t(ui_language || 'zh-CN', 'delete_btn')}
                    </button>
                  </div>
                </div>
              ))}
              <div className="form_row" style={{ gap: 8 }}>
                ...
                >
                  {t(ui_language || 'zh-CN', 'new_template_btn')}
                </button>
                <button
                  className="settings_btn"
                  onClick={async () => {
                ...
                >
                  {t(ui_language || 'zh-CN', 'export_templates_btn')}
                </button>
                <button
                  className="settings_btn"
                  onClick={async () => {
                ...
                >
                  {t(ui_language || 'zh-CN', 'import_templates_btn')}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="modal_footer">
          <div style={{ flex: 1 }} />
          <button className="settings_btn" onClick={on_save}>
            {t(ui_language || 'zh-CN', 'save_btn')}
          </button>
        </div>
      </div>
    </div>
...
const SettingsModal = memo(SettingsModalComponent)
